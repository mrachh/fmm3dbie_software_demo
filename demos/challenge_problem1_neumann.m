...
dir = [0,0,1];
rhs = helm3d.planewave(zk,dir,S);
rhs = rhs.*(1i*zk*(S.n(1,:)*dir(1)+S.n(2,:)*dir(2)+S.n(3,:)*dir(3))).';


eps = 1E-4;

t1 = tic; sig = helm3d.neumann.solver(S,rhs,eps,zk,alpha); tend = toc(t1);
...
srcinfo.sources = S.r;
srcinfo.nd      = 1;
srcinfo.charges = 0*S.wts.';
srcinfo.dipoles = S.n.*(S.wts.');
[U] = lfmm3d(epsfmm,srcinfo,0,targ_info.r,1);
inout = U.pottarg;
indsin = find(abs(inout)>1E-1);
indsou = find(abs(inout) <= 1E-1);



targ_info.r = targ_info.r(:,indsou);
t2 = tic;
eps = 1E-4;
pot = helm3d.neumann.eval(S,sig,targ_info,eps,zk,alpha); tend = toc(t2);
...
utot = pot_in-pot;
utot_plt = zeros(size(xyz_out,2),1);
utot_plt(indsou) = utot;
utot_plt(indsin) = NaN;
utot_plt = reshape(utot_plt,sz);
